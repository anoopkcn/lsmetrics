...
class LatentSpaceVisualizer:
    ...
    def plot_tsne(
        ...
        if labels is not None:
            scatter = ax.scatter(
                embeddings_2d[:, 0], embeddings_2d[:, 1], c=labels, cmap="viridis"
            )
    ...
    @staticmethod
    def plot_distance_distribution(embeddings: torch.Tensor, ax: Optional[Axes] = None):
    ...
    @staticmethod
    def plot_metrics_summary(metrics: Dict[str, float], ax: Optional[Axes] = None):
    ...
    def create_summary_plot(
        ...
        if "local_smoothness_mean" in metrics:
            ...
            data = (
                [metrics["local_smoothness_mean"]]
                if isinstance(metrics["local_smoothness_mean"], float)
                else metrics["local_smoothness_mean"]
            )
